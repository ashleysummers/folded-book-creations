...
function sliceCategories(slice) {
  ...
  // Legacy schema: single category stored on the primary tab
  const primaryCat = slice.primary?.category;
  if (primaryCat) {
    if (typeof primaryCat === 'string') {
      categories.add(primaryCat);
    } else if (Array.isArray(primaryCat) && primaryCat.length && primaryCat[0].text) {
      categories.add(primaryCat[0].text);
    }
  }

  // New schema: repeater field (one item per category)
  if (Array.isArray(slice.items)) {
    slice.items.forEach(item => {
      ...
      if (typeof c === 'string') {
        categories.add(c);
      } else if (Array.isArray(c) && c.length && c[0].text) {
        categories.add(c[0].text);
      }
    });
  }
...
export default function Home(props) {
  ...
  document.data.body.forEach(slice => {
    ...
    const names = sliceCategories(slice);
                  ...
                  return (
                    <div className="col-sm-6 col-md-4 ws-works-item" key={index} style={{ transitionDelay: `${(index % 6) * 70}ms` }}>
                      ...
                      <div className="ws-works-caption text-center">
                        ...
                        {slice.primary.price !== undefined && slice.primary.price !== null && (
                          (() => {
                            const priceStr = Number(slice.primary.price).toFixed(2);
                            const [dollars, cents] = priceStr.split('.');
                            return (
                              <div className="ws-item-price" style={{ marginTop: '6px' }}>${dollars}<sup style={{ marginLeft: '2px' }}>{cents}</sup></div>
                            );
                          })()
                        )}
